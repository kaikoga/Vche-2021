module LoyaltyTools
  ...
  def user_is_source?(event, user)
    event_has_owner?(event) ? user_is_backstage_member?(event, user) : user_is_creator?(event, user)
  end

  def user_is_primary_source?(event, user)
    event_has_owner?(event) ? user_is_owner?(event, user) : user_is_creator?(event, user)
  end
end
